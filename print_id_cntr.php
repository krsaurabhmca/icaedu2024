...
$cntr_id = get_data('student',$sid,'center_id')['data'];
$cntr = get_data('center_details',$cntr_id,'center_logo')['data'];
// print_r($cntr);

if(isset($_POST['printqr']))
{
	//$qrvalue = 'student_id='.$sinfo['id'].'&student_roll='.$sinfo['student_roll'];
	$qrvalue = $sinfo['student_name'].''.$sinfo['student_roll'].'%20https://icaedu.co.in/registration-verification';
	//$qrvalue = encode($qrvalue);
	$url ="http://chart.apis.google.com/chart?cht=qr&chs=90x90&chl=".$qrvalue."&chld=H|0"; 
	$url = qrcode($qrvalue);
//	echo " <div class='qr'>". $sinfo['student_roll']."<img src ='". $url."' width ='100px' border=''/></div>"; 

?>
<style type="text/css">
	.idcard{width:200px; height:325px; background:url('temp/upload/<?php echo $cntr ?>') no-repeat; text-align:center; float:left; margin:4px;margin-top:30px;page-break-before: always;position:relative;}
</style>
<div class='idcard2'>
...
<table border='0' class='idcard' bordercolor='red' cellspacing='0'	>
	...
	<tr height='70px'>
		<td> <span style='color:red;font-family:times new roman;font-size:12px;'> Study Center Address </span><br>
			<b style='font-size:11px'><?php //echo centerinfo($sid,'center_name'); ?></b>  
		   ...
		   <small>Visit: www.<?php echo $inst_domain; ?> </small> <br>
