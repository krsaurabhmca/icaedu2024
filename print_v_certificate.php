...
*/
			...
			<table width='1050px' height='420' border='0'  >
				...
				<td valign='top' style='padding-right:0px;padding-top:40px;text-align:center;font-family:times new roman;font-size:18px;'> Sr. No. : <?php  $msid= resultinfo($sid,'cer_no');  print_r($msid);
				$qrvalue = "Reg No :". $studentinfo['student_roll']."Name". $studentinfo['student_name'] ."Certificate No". resultinfo($sid,'cer_no') . "http://icaedu.co.in/result.php";
				...
				echo $studentinfo['student_father']; ?>, Reg No. <?php echo $studentinfo['student_roll']; ?> on successfully completion of <span style='color:blue;font-weight:400;'><?php echo courseinfo($sid,'course_name'); ?> </span>
				 ( Duration - <?php  echo courseinfo($sid,'course_duration'); ?> Months ) Course and secured <?php echo resultinfo($sid,'percentage'); ?> % </span> with Grade <?php echo resultinfo($sid,'grade'); ?><sup>*</sup>from our authorised Study Centre <?php echo centerinfo($sid,'center_name'); ?>,  <?php echo centerinfo($sid,'center_address'); ?>, <?php  echo get_data('district',centerinfo($sid,'dist_code'),'dist_name','id')['data']; ?>, <?php echo get_data('state',centerinfo($sid,'id'),'state_name','state_code')['data']; ?>  Centre Code <?php echo centerinfo($sid,'center_code'); ?>   
				 </div>
				 </td>
				...
				<td height='120px' valign='bottom' > <img src='syllabus/irshad_sign.png' alt='Signature' width='120px' style='padding:20px;'>
				</td>
				</tr>				
