...
// 		$result = get_multi_data($table_name, array( 'status'=>'AUTO'), ' order by id desc limit 1');
...
function upload_doc ($file_name , $imgkey = 'rand', $target_dir = "upload")
    {
		if (!file_exists($target_dir)) {
			mkdir($target_dir, 0755, true);
		}
        ...
        // }
        // Allow certain file formats
        // if($imageFileType != "jpg" && $imageFileType != "png" && $imageFileType != "jpeg" && $imageFileType != "gif" && $imageFileType != "docx" && $imageFileType != "xlsx" && $imageFileType != "pptx" && $imageFileType != "accdb" && $imageFileType != "pmd" && $imageFileType != "cdr" && $imageFileType != "psd" ) {
        //     $res['msg']= "Sorry, only JPG, JPEG, PNG DOCX, XLSX, PPTX, ACCDB, PMD, CDR, PSD & GIF files are allowed $imageFileType.";
        //     $uploadOk = 0;
        // }
        // Check if $uploadOk is set to 0 by an error
        if ($uploadOk == 0) {
...
function create_qr($data) {
    // API endpoint
    $apiEndpoint = 'https://qr.morg.in/api.php';
...
function send_sms($number,$sms,$templateid=''){
    global $base_url;
    global $wa_api_key;
    $link='';
    $onlysms=false;
    if($templateid !=''){
    $resp = send_msg($number,$sms,$templateid);
    $onlysms=true;
    }
    if($templateid == false){
        $onlysms=true;
    ...
    //  return true;
    $msg = urlencode($sms);
    $url = "https://wa.x2z.in/get_wa.php?wa_api_key={$wa_api_key}&link={$base_url}{$link}&number={$number}&msg=$msg&onlysms=$onlysms";
...
function btn_delete($table, $id,  $disabled ="" , $icon = 'fa fa-trash' , $per = 'no')
		{
			$str ="<button class='delete_btn btn btn-danger btn-sm'  data-table='$table' data-id='$id' data-pkey='id' data-per='$per' title='Detete This Permanently' $disabled > <i class='$icon'></i> </button> ";
			return $str ;
		}
function btn_dlt($table, $id,  $disabled = "")
{
	global $user_id;
	global $user_type;
	global $user_name;
...
function btn_edit($page_url, $id , $icon = 'fa fa-edit',$title ='Edit Information',$color = 'info')
		{
			$link =$page_url."?link=".encode("id=".$id);
			$str ="<a href='$link' class=' btn btn-$color btn-sm text-light' title='$title'> <i class='$icon'></i> </a> ";
			return $str ;
		}
		
function btn_view($table, $id ,$title ='' ,$icon = 'fa fa-eye')
		{
			$view_link = 'view_data.php?link='.encode('table='.$table.'&id='.$id);
			$str ="<a data-href='$view_link' class='view_data btn btn-success btn-sm text-light' data-title='$title'><i class='$icon'></i></a> ";
			return $str ;										
		}
...
function dropdown_where($table_name,$id,$list,$whereArr, $selected =null)
	{
			...
			$sql = "select * from ".  $table_name ." WHERE " .implode('and ', $where) ." order by $list";
			$res=mysqli_query($con,$sql) or mysqli_error($con);
...
function dropdown_two_value($tablename,$value,$list,$selected =null, $list2=null)
		{
    		global $con;
    		$i =0;
    		$query ="select * from $tablename where status ='ACTIVE' order by $list";
    		...
    		?>
    		<option value='<?php echo $key ?>' data-price='<?php echo $col2 ?>' <?php if($key ==$selected) echo "selected";?> ><?php echo $show //." - ". $col2; ?></option>
    		<?php
    		}		
		}
...
function dropdown_list_multiple($tablename,$value,$list,$selectedArr =null)
		{
    		...
    		while($row =mysqli_fetch_array($res))
    		{
                ...
                ?>
				<option value='<?php echo $key; ?>' <?php if($found >-1) echo "selected"; ?> ><?php echo $show; ?></option>
				<?php
    		}		
		}
...
function get_rank($sql, $col_name , $col_value)
  {
...
function wa_button($link,$number,$msg,$img=null,$title=null,$bsClr='success'){
    global $inst_wa;
    $pdf="";
    if($inst_wa == 'off'){
        $btn = "";
    }else{
        $btn = "<a onclick='wa_send(this)' data-link='$link' data-number='$number' data-msg='$msg' data-pdf='$pdf' data-img='$img' title='$title' class='mx-1 btn btn-sm btn-$bsClr'><i class='fa fa-share-square' aria-hidden='true'></i></a>";
    }
	return $btn;
}

function wa_all_send($number, $msg, $pdf=null,$img=null){
    global $wa_api_key;
    global $base_url;
    $pdf_path ="";
    $img_path="";
...
*".$inst_name."*";
    ...
    $msg = urlencode($msg);
    if($pdf !=null){
       $pdf_path = "&pdf={$base_url}{$pdf}";
    }
    if($img !=null){
      $img_path = "&img1={$base_url}{$img}";
    }
    
    $url = "http://148.251.129.118/whatsapp/api/send?mobile={$number}&msg=$msg&apikey={$wa_api_key}{$img_path}{$pdf_path}";
    
    return $res = api_call($url);
}

// function wa_send($link,$number,$msg,$img=null){
//     global $base_url;
//     global $user_id;
//     $dir = '../pdf_shere/';
//     $fname = 'pdf_shere/'.date('ymdhis').$user_id.'.pdf';
//     $leave_files = array(date('ymdhis').$user_id.'.pdf');
//     $url = $base_url.$link.'&download=1&fname='.$fname;
// 	api_call($url);

// 	$res = wa_all_send($number, $msg, $fname,$img);
...
// function send_pdf($link,$number,$msg){
//     global $base_url;
//     global $wa_api_key;
//     $msg = urlencode($msg);
//     $url = "https://wa.x2z.in/get_wa.php?wa_api_key={$wa_api_key}&link={$base_url}{$link}&number={$number}&msg=$msg";
//     $res = api_call($url);
// 	return $res;
// }

function send_pdf($link,$number,$msg){
   wa_send($number, $msg, $link);
}

function array_to_comma($z)
{
  $ab = array_filter($z);
...
//SEND WHATSAPP WITHOUT WAITING FOR RESPONCE 

function wa_send($mobile, $msg, $file_url ='', $file_name ='' ){
   	global $instance_id;
	global $access_token;
    ...
    $file_link = ($file_url !='')?"&pdf=".$file_url.".pdf":"";
    
$msg = ($file_url !='')? urlencode($msg. " 
Click to Download 👇
"). create_sl($file_url) : urlencode($msg) ;
    //$api_url="https://wa.pinkcarpet.in/api/send?number=91{$mobile}&type=media&message=$msg&media_url=$file_url&filename=$file_name&instance_id=$instance_id&access_token=$access_token"; 
    //$api_url="https://wa.pinkcarpet.in/api/send?number=919431426600&type=media&message=$msg&media_url=$file_url&filename=$file_name&instance_id=$instance_id&access_token=$access_token"; 
    $api_url="http://148.251.129.118/wapp/api/send?apikey=c354b1dac58c488e9eb16f1b86d3cdc7&mobile=91{$mobile}&msg={$msg}";//{$file_link}";
    //$api_url="http://148.251.129.118/wapp/api/send?apikey=c354b1dac58c488e9eb16f1b86d3cdc7&mobile=919431426600&msg={$msg}"; //{$file_link}";
    $ch = curl_init($api_url);
    curl_setopt($ch,CURLOPT_RETURNTRANSFER,1);
    curl_setopt($ch, CURLOPT_CONNECTTIMEOUT, 5); // Set a timeout here
    $response = curl_exec($ch);

    if ($response === false) {
        return null;
        // API call failed due to timeout, handle the error or ignore it
    } else {
...
function wa_send_old($number, $message, $media_url='', $doc_name ='' ) {
  global $instance_id;
  global $access_token;
    $api_url = 'https://wa.pinkcarpet.in/api/send';

  //  insert_data('wa_sms',['mobile'=>'91'.$number, 'msg' =>$message,'status'=>'PENDING']);
    ...
    if(strlen($number)==10)
    {
      ...
      if (!filter_var($media_url, FILTER_VALIDATE_URL) === false) {
        ...
        curl_setopt($ch, CURLOPT_POSTFIELDS, $json_data);
         ...
         // Set timeout to a low value so the request doesn't wait for long
        curl_setopt($ch, CURLOPT_TIMEOUT, 1); // Set the timeout in seconds
